...
public class Hands : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(mouseRay, out raycastInfo, float.MaxValue, collisionLayer)){
            if (raycastInfo.point.x > -1.5 && raycastInfo.point.x < 1.5f && rightHand.state != Hand.STATE.REACHING_MIDDLE && rightHand.state != Hand.STATE.MIDDLE)
            {
            ...
            else if (raycastInfo.point.x < -1.5f && leftHand.state != Hand.STATE.REACHING_SIDE && leftHand.state != Hand.STATE.SIDE)
            {
